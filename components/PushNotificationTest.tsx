...
import { getPushToken, sendPushNotification } from '@/services/messagingService';
    ...
    const fetchToken = async () => {
      try {
        ...
        if (token) {
          ...
          console.log('No token obtained - check if running on physical device');
        }
      } catch (error) {
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50">
      <View className="p-4 pt-12">
        ...
        <Text className="text-sm font-medium mb-2">Platform: {Platform.OS}</Text>
        <Text className="text-sm font-medium mb-2">Device: {Platform.OS === 'ios' ? (Platform as any).isPad ? 'Tablet' : 'Phone' : 'Phone'}</Text>
        ...
        <View className="mt-6 p-4 bg-blue-50 rounded-md">
          ...
          <Text className="text-xs text-gray-700 mb-1">• Must be on physical device (not simulator)</Text>
          <Text className="text-xs text-gray-700 mb-1">• Grant notification permissions when prompted</Text>
          <Text className="text-xs text-gray-700 mb-1">• Copy token from one device and paste into another</Text>
          <Text className="text-xs text-gray-700">• Check console logs for detailed error info</Text>
        </View>
      </View>
    </ScrollView>
  );
