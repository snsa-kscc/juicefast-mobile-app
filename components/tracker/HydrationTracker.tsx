...
export function HydrationTracker({ initialWaterData, onBack }: HydrationTrackerProps) {
  const { user } = useUser() || {};
  ...
  const deleteWaterEntry = useMutation(api.waterIntake.deleteByUserIdAndTimestamp);
  ...
  return (
    <ScrollView className="flex-1 bg-[#FCFBF8]">
      <WellnessHeader title="Hydration Tracker" subtitle="Stay hydrated, stay healthy — every drop counts." accentColor="#4FC3F7" showBackButton={true} onBackPress={onBack} />

      <TrackerStats title="DAILY WATER INTAKE" subtitle={`${displayedWater}ml out of ${DAILY_GOAL}ml`}>
        <CircularProgress
        ...
        <View className="flex-row justify-between mb-4">
          <TouchableOpacity 
            className={`border rounded-md px-3 py-2 ${
              waterAmount === 250 ? 'border-[#4FC3F7] bg-[#E1F5FE]' : 'border-gray-300'
            }`}
          ...
          <TouchableOpacity 
            className={`border rounded-md px-3 py-2 ${
              waterAmount === 500 ? 'border-[#4FC3F7] bg-[#E1F5FE]' : 'border-gray-300'
            }`}
          ...
          <TouchableOpacity 
            className={`border rounded-md px-3 py-2 ${
              waterAmount === 750 ? 'border-[#4FC3F7] bg-[#E1F5FE]' : 'border-gray-300'
            }`}
            ...
            {waterEntries.map((entry, index) => {
              ...
              return (
                <View key={entry._id} className="flex-row justify-between items-center py-2 border-b border-gray-100 last:border-b-0">
                  <View>
                    <Text className="font-lufga text-sm font-medium">{entry.amount}ml</Text>
      ...
      <View className="px-6 mt-6 mb-20">
        <View className="bg-white rounded-lg p-4 shadow-sm">
          ...
          <View className="space-y-2">
            {["Start your day with a glass of water", "Keep a water bottle with you at all times", "Set reminders to drink water every hour"].map(
              (tip, index) => (
