...
const useWarmUpBrowser = () => useEffect(() => {
...
export const useSocialSignIn = () => {
  ...
  const signInWithGoogle = useCallback(async (onSignupComplete?: () => Promise<void>) => {
    await signInWith('oauth_google', onSignupComplete)
  }, [signInWith])

  const signInWithFacebook = useCallback(async (onSignupComplete?: () => Promise<void>) => {
    await signInWith('oauth_facebook', onSignupComplete)
  }, [signInWith])

  const signInWithApple = useCallback(async (onSignupComplete?: () => Promise<void>) => {
